...
![컨테이너 vs 가상머신(VM) 구조 비교 — VM은 게스트 OS 포함, 컨테이너는 커널 공유](../picture/container-vs-vm.svg)
...
![볼륨과 bind mount — 컨테이너 삭제 시 데이터 소멸 vs 볼륨으로 데이터 유지](../picture/volume-bind-mount.svg)
