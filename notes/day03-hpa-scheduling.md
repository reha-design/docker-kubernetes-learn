...
### "장부"와 실제 분배는 서로 다른 층이다 — cgroup 매핑

위 "자원 장부"는 **스케줄링 시점**의 회계일 뿐, 노드의 물리 자원을 미리 잘라서 파드마다
나눠주는 게 아니다. 실제로 파드가 그 자원을 "나눠 쓰는" 메커니즘은 완전히 다른 층에서
일어난다 — 파드가 뜰 때 kubelet이 컨테이너 런타임에게 **Linux cgroup**을 설정해달라고
지시하고, requests/limits가 이렇게 번역된다:
...
공간을 되돌릴 수 없는 **압축 불가능(incompressible)** 자원이라 초과 시 강제 종료 말고는
방법이 없다. 노드 하나에 물리 코어가 8개뿐이어도 그 위에서 파드 수십 개가 동시에 도는 게
