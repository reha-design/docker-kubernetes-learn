...
**결과 해석**: 초반엔 대기 시간이 계속 늘어나다가(대략 2배씩) 마지막엔 정확히 **300초(5분)에서
고정**됐다. **상한 300초와 "2배씩 증가 후 상한 고정"이라는 패턴은 실측으로 확인됐지만,
절대값은 문서상 수열(`10s→20s→40s…`)과 일치하지 않는다** — 여기서 잰 값은 순수 백오프
대기 시간이 아니라 `AGE` 차이, 즉 **백오프 대기 + 이미지 pull + 컨테이너 생성/실행/종료
시간까지 합산된 값**이기 때문이다(위 Events에서 재시작마다 `Pulling image`가 찍힌 것도
이 오차에 포함). 실측에서 확인할 수 있는 건 "증가 추세와 300초 상한"까지이고, 정확한
초기값 10초는 문서/소스 기준으로 인용해야 한다.
...
2. CrashLoopBackOff의 재시작 간격은 10s→20s→40s→80s→160s→300s(상한)의 지수 백오프이며,
   실측에서도 "2배씩 증가하다 300초에서 고정"되는 패턴이 확인됐다(절대값은 pull·실행 시간이
   섞여 문서 수열과 다름). Job의 backoffLimit과 달리 재시도를 포기하지 않는다
